  ...
  <div class="min-h-screen bg-brand-dark text-white flex flex-col items-center justify-center p-4 relative overflow-hidden">
    ...
    <div class="absolute top-6 left-0 w-full px-6 flex justify-between items-center z-50">
      <NuxtLink to="/squad" class="flex items-center gap-2 text-gray-400 hover:text-white transition-colors group">
        ...
        <span class="text-sm font-bold font-display tracking-wide">LEAVE</span>
      </NuxtLink>
    </div>

    <div v-if="status === 'waiting'" class="z-10 w-full max-w-sm text-center space-y-8 animate-fade-in">
      <div>
        <p class="text-gray-500 text-xs uppercase tracking-[0.3em] font-bold mb-4">Room Code</p>
        ...
        <button @click="copy(shareUrl)" 
          ...
          <span :class="copied ? 'text-green-500' : 'text-gray-400'">{{ copied ? 'Copied Link' : 'Copy Link' }}</span>
        </button>
      </div>

      <div class="glass rounded-3xl p-8 space-y-6">
        <h3 class="font-bold text-gray-400 text-xs uppercase tracking-widest">Squad Members</h3>
        <div class="flex flex-wrap justify-center gap-3">
          ...
          <span v-if="participants.length === 0" class="text-gray-600 text-sm italic">Waiting for players...</span>
        </div>
      </div>
    ...
    <div v-else-if="status === 'active'" class="z-10 w-full max-w-sm flex flex-col items-center gap-8 animate-slide-up">
      <div class="text-center">
        <h2 class="text-2xl font-bold font-display text-gray-300">Swipe Together</h2>
      </div>
      ...
      <div v-else class="w-full max-w-sm h-[540px] glass rounded-[2rem] flex flex-col items-center justify-center p-8 text-center space-y-6">
        ...
        <div>
          <h3 class="text-xl font-bold font-display mb-2">No Match Yet?</h3>
          <p class="text-sm text-gray-400">Waiting for others to vote or maybe you need more options.</p>
        </div>
        <button @click="loadMoreCards" :disabled="loadingMore"
          ...
          <span>{{ loadingMore ? 'Loading...' : 'Load More Cards' }}</span>
        </button>
      </div>
    </div>
    ...
    <div v-else-if="status === 'matched'" class="z-10 text-center space-y-8 w-full max-w-sm animate-fade-in">
      ...
      <h1 class="text-5xl font-black font-display text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-green-600 leading-tight">It's a Match!</h1>
